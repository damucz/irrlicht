...
struct SEvent
{
    ...
    {
        // Whether the touch is active
        u32 Touched[NUMBER_OF_MULTI_TOUCHES];

        // X position of the touches.
        ...
        //! Type of touch event.
        EMULTITOUCH_INPUT_EVENT Event;

        u32 touchedCount() const { u32 count = 0; for (int i = 0; i < NUMBER_OF_MULTI_TOUCHES; ++i) if (Touched[i] > 0) ++count; return count; }
        void clear() { for (int i = 0; i < NUMBER_OF_MULTI_TOUCHES; ++i) Touched[i] = 0; }
    };
